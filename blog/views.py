...
def get_blog_common_data(request,blogs_all_list):
    ...
    paginator = Paginator(blogs_all_list,settings.ALLBLOGS_NUMBER)
    page_num = request.GET.get('page',1)
    page_of_blogs = paginator.get_page(page_num)
    currentr_page_num = page_of_blogs.number
    page_range = [x for x in range(currentr_page_num-2,currentr_page_num+3) if x in paginator.page_range]
